...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("machtcoin"))
        return false;
...
{
    // Convert machtcoin:// to machtcoin:
    //
    //    Cannot handle this later, because machtcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("machtcoin://"))
    {
        uri.replace(0, 12, "machtcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MachtCoin.lnk";
}
...
{
    return GetAutostartDir() / "MachtCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MachtCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  MachtCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("MachtCoin-Qt"));
