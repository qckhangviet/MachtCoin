...
#if defined(NDEBUG)
# error "MachtCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "MachtCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	// Premine 8.000.000 for ICO
	// After height 25.000, no more PoW coin will be rewarded
	if(pindexBest->nHeight == 0) {
		nSubsidy = 8000000 * COIN;
	} else if (pindexBest->nHeight >= 1 && pindexBest->nHeight <= 25000) {
		nSubsidy = 200 * COIN;
	} else {
		nSubsidy = 0;
	}
...
#ifdef ENABLE_WALLET
// MachtCoin: attempt to generate suitable proof-of-stake
...
{
	RenameThread("MachtCoin-loadblk");
